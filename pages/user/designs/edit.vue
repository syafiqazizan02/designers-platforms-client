...
export default {
  ...
  // fecth form database in server side (get axios, id, error, url)
  async asyncData({ $axios, params, error, redirect }) {
    try {
      const design = await $axios.$get(`/designs/${params.id}/byUser`); // get design
      const teams = await $axios.$get(`/users/teams`); // get users by team
      return {
        design: design.data,
        teams: teams.data
      };
  ...
  mounted() {
    // check if no design
    if (this.design) {
      // access data for pre-populate (return loop into form)
